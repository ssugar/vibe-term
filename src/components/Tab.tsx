...
export function Tab({
  ...
  switch (displayMode) {
    ...
    case 'medium': {
      ...
      const truncatedName =
        rawName.length > mediumMaxWidth ? rawName.slice(0, mediumMaxWidth - 1) + '\u2026' : rawName;
    ...
    default: {
      ...
      const truncatedName =
        rawName.length > maxNameWidth ? rawName.slice(0, maxNameWidth - 1) + '\u2026' : rawName;
      tabContent = `[${indexDisplay}:${truncatedName} ${statusEmoji} `;
      closingBracket = ']';
      break;
    }
  ...
  if (isSelected) {
    return (
      <Text>
        ...
        <Text underline color={contextColor}>
          {contextPct}
        </Text>
