/**
 * Service for managing the ~/.vibe-term/ directory.
 * Provides utilities for creating the directory, reading/writing files,
 * and installing the hook script for Claude Code integration.
 */
...
/**
 * Path to the vibe-term configuration directory
 */
export const VIBE_TERM_DIR = join(homedir(), '.vibe-term');

/**
 * Ensure the ~/.vibe-term/ directory exists.
 * Creates it with recursive: true so parent directories are created if needed.
 */
...
/**
 * Get the path to the vibe-term directory.
 */
...
/**
 * Get the full path to a file in the ~/.vibe-term/ directory.
 */
...
/**
 * Options for writing a file to the vibe-term directory.
 */
export interface WriteFileOptions {
  /** If true, set the file as executable (chmod 755) */
  executable?: boolean;
}

/**
 * Write a file to the ~/.vibe-term/ directory.
...
/**
 * Read a file from the ~/.vibe-term/ directory.
 * @returns The file content, or null if the file doesn't exist.
 * @throws For errors other than file not found.
 */
...
/**
 * Check if a file exists in the ~/.vibe-term/ directory.
 */
...
/**
 * The hook script content for Claude Code integration.
 * This script is written to ~/.vibe-term/status-hook.sh
 */
...
/**
 * Install the hook script to ~/.vibe-term/status-hook.sh
 * @returns The path where the script was installed.
 */
export async function installHookScript(): Promise<string> {
  return writeVibeTermFile('status-hook.sh', HOOK_SCRIPT_CONTENT, { executable: true });
}
