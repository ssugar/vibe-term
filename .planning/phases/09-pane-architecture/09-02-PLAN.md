...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app.tsx"
      to: "src/services/paneSessionManager.ts"
      via: "import and call createSessionPane for lazy pane creation"
      pattern: "createSessionPane.*projectPath"
...
<task type="auto">
  <name>Task 3: Implement Enter key session switching with lazy pane creation</name>
  <files>src/app.tsx</files>
  <action>
Update the Enter key handler to switch sessions using paneSessionManager, with defensive pane creation.
...
2. Replace the existing Enter key handler (around line 145-159) with pane-based switching that lazily creates panes:
          ...
          .then(({ stdout }) => {
            ...
            return execAsync(`tmux list-panes -F '#{pane_id}'`)
              .then(({ stdout: paneList }) => {
                ...
                // CRITICAL: Check if session pane exists, create if not
...
Note: The b key handler (returnToHud) can stay for now as a fallback, but Ctrl+h is the primary return mechanism.
  ...
  <verify>
TypeScript compiles: `npm run build`
Grep for pane management imports: `grep -E "(switchToSession|createSessionPane|getSessionPane|ensureScratchWindow)" src/app.tsx`
  </verify>
  <done>
Enter key triggers pane-based session switching with lazy pane creation: checks if pane exists via getSessionPane(), creates via ensureScratchWindow()+createSessionPane() if not, then switches. Updates activeSessionId in store on success.
  </done>
</task>
...
- [ ] app.tsx imports switchToSession, getSessionPane, createSessionPane, ensureScratchWindow
- [ ] app.tsx uses getSessionPane to check if pane exists before switching
- [ ] app.tsx calls createSessionPane when pane doesn't exist
...
2. Enter switches session (creating pane if needed) and updates active marker
