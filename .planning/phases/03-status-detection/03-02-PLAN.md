...
3. Update the Session object creation to use detected status and model:
   ...
   - **REQUIRED for type compatibility:** Convert null model to default using `model ?? 'sonnet'`
     ```typescript
     model: model ?? 'sonnet'
     ```
     This is REQUIRED because:
     - getSessionStatus returns `{ status: SessionStatus; model: ModelType | null }`
     - types.ts defines Session.model as `'sonnet' | 'opus' | 'haiku'` (non-nullable)
     - Without this conversion, TypeScript will error: "Type 'null' is not assignable to type..."
...
Confirm current code structure before modifying:
- Line ~133 should show: `return sortSessions(sessions, previousOrder);`
- The existing sortSessions function should be at lines ~46-77
If these don't match, adjust line references accordingly.
...
4. Update buildSessions() return statement (currently at line ~133):
   - Replace: `return sortSessions(sessions, previousOrder);`
   - With: `return sortSessionsWithBlocked(sessions, previousOrder);`
