...
must_haves:
  truths:
    - "HUD can read session status from Claude's logs"
    - "HUD can identify which model a session is using"
    - "HUD gracefully handles missing or unreadable log files"
  artifacts:
...
1. `extractModel(modelString?: string): 'sonnet' | 'opus' | 'haiku' | null`
   - If no modelString or empty string, return null
   - Check lowercase string with includes():
     - Contains 'opus' -> return 'opus'
     - Contains 'haiku' -> return 'haiku'
     - Contains 'sonnet' -> return 'sonnet'
   - If none match (unknown model), return null
   - Note: Returning null allows UI to show "unknown" for unrecognized models
...
3. `getSessionStatus(projectPath: string): { status: SessionStatus; model: ModelType | null }`
   - ModelType is 'sonnet' | 'opus' | 'haiku'
   - Call findLatestLogFile(projectPath)
   - If no log file, return { status: 'idle', model: null } (defaults - null indicates unknown)
   ...
   - Call extractModel(entry.message?.model) - returns null if unknown
   - Return { status, model }

Import LogEntry from './logPathService.js'. Use Session types from '../stores/types.js' for SessionStatus type alias.
  </action>
  <verify>TypeScript compiles: `npx tsc --noEmit`</verify>
  <done>statusDetector.ts exists with getSessionStatus export that returns status and model (model may be null)</done>
</task>
...
- Model extraction handles sonnet/opus/haiku variants, returns null for unknown
