...
must_haves:
  truths:
    ...
    - "Panes for dead internal sessions are cleaned up (killed)"
    - "External session panes are NOT cleaned up (they belong to other tmux sessions)"
...
Purpose: When Claude processes exit (user types /quit, session ends), the HUD should automatically remove the session from display and clean up the associated tmux pane. If the active session dies, focus should return to the HUD pane. External sessions (detected running in other tmux sessions) should NOT have their panes cleaned up.
...
2. Add ref to track previous sessions with their isExternal flag (after line 18):
   ```typescript
...
IMPORTANT: Only call cleanupSessionPane for internal sessions (isExternal === false).
External sessions have panes in other tmux sessions - cleaning them up would kill
panes that don't belong to us.
  </action>
  <verify>
...
External sessions (detected via scanner) should NOT have panes killed when they disappear.
  </verify>
  <done>
useSessions detects removed sessions and calls cleanupSessionPane only for internal sessions. External session panes are preserved.
  </done>
</task>
...
- [ ] Dead internal sessions disappear from HUD within 2-3 seconds
- [ ] Panes for dead internal sessions are killed (no orphan panes in scratch window)
- [ ] External session panes are NOT killed when external sessions disappear
- [ ] If active session dies, focus returns to HUD
- [ ] No error messages displayed during cleanup
</verification>

<success_criteria>
1. Internal sessions automatically removed when Claude process exits
2. Associated tmux panes for internal sessions cleaned up silently
3. External session panes are never cleaned up (they belong to other tmux sessions)
4. Active session death handled gracefully with focus return to HUD
</success_criteria>
